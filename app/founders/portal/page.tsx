...
export default async function ProviderPortalHome() {
  ...
  if (!isDemoMode()) {
    ...
    if (!data.user) redirect('/founders/portal/login?next=/founders/portal');
    user = data.user;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Tile title="Submit a new pre-auth" body="Walk through the required fields for outpatient, inpatient, meds, home health, therapy, or DME." href="/founders/portal/submit" />
        <Tile title="My submitted cases" body="Track status, see determinations, download letters." href="/founders/portal/cases" disabled />
        <Tile title="Office settings" body="Update fax, email, contact info. Add staff with portal access." href="/founders/portal/settings" disabled />
      </div>
