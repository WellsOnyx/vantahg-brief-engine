...
import type { EligibilityResult } from '@/lib/founders/eligibility';
...
export default function ConciergeCallIntake() {
  ...
  async function runEligibility() {
    ...
    try {
      const res = await fetch('/api/founders/eligibility', {
        method: 'POST',
  ...
  async function submitIntake() {
    ...
    setSubmitError(null);
    try {
      const res = await fetch('/api/founders/intake', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          client_id: clientId,
      ...
      router.push(`/founders/cases/${data.case_id}?intake=concierge`);
    } catch (err) {
