...
/**
 * First Mover concierge intake submission.
 *
...
export async function POST(request: NextRequest) {
  ...
  // Real-PHI gate: enforce First Mover's synthetic-only posture until BAA is signed
  if (process.env.FIRSTMOVER_ALLOW_PHI === 'true' && !isDemoMode() && !hasSupabaseConfig()) {
    return NextResponse.json(
      { error: 'Server misconfigured: FIRSTMOVER_ALLOW_PHI=true but Supabase not configured.' },
      { status: 500 }
    );
  }
  ...
  if (isDemoMode() || !hasSupabaseConfig()) {
    const case_id = `demo-firstmover-${Date.now()}`;
  ...
  await logAuditEvent(
    created.id,
    'firstmover_intake_opened',
    'firstmover_concierge',
    {
      service_type,
      intake_channel,
