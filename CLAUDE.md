...
12. De-identified review training dataset — as clinicians finalize determinations, `lib/dataset/review-dataset.ts:captureReviewSample` auto-captures a HIPAA Safe Harbor de-identified sample pairing the review INPUTS (clinical brief, extracted clinical data, codes, clinical question) with the human LABEL (determination + rationale + criteria + tier decisions + physician AI-agreement). One row per case in `review_training_samples` (migration 027), upserted so the label always reflects the latest human decision. `lib/deident/safe-harbor.ts` does the scrub: targeted redaction of each case's known identifiers + a regex safety net for the 18 Safe Harbor identifier shapes, DOB→bucketed age (90+ aggregated), dates→year, while preserving CPT/ICD codes. Hooked fire-and-forget into the RN-approve path, MD determination PATCH, attorney determination, and physician feedback. Nightly cron `/api/cron/review-dataset-backfill` self-heals + retroactively covers historical reviews. Admins export JSONL/JSON via `GET /api/admin/review-dataset` (case_id never leaves — only the pseudonymous `sample_ref`). Toggle `REVIEW_DATASET_INCLUDE_FREETEXT=false` for a structured/coded-only dataset. This is the foundation for training VantaUM's own fallback model on its own reviews.
13. 340+ passing tests (prior suite + 22 for Safe Harbor de-identification + 12 for dataset assembly/capture + 5 for review-dataset endpoints)
