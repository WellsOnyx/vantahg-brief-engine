...
12. Standards rails (inbound) — `POST /api/connect/fhir/Claim/$submit` (Da Vinci PAS: Bundle → CanonicalCase → shared ingest → ClaimResponse queued/A4 with preAuthRef) and `POST /api/connect/x12/278` (005010X217 subset: EDI → CanonicalCase → shared ingest → mirrored 278 response with HCR action + auth number). Both partner-key-authed and funneled through `lib/partner/ingest.ts:ingestCanonicalCase` (ledger idempotency, dedup, tenant-from-key, brief queue). See `docs/CONNECTOR_RAILS.md`.
13. 300+ passing tests (211 from prior + 13 for triage queue API + 12 for determination delivery + 11 for portal endpoints incl. cross-tenant invite guard + 19 for iCal generator and kickoff invite + 6 for case-documents upload + 7 for case-documents signed-URL + 12 for SLA-aware LPN scoring + 12 for quality audit endpoints + 33 for standards rails)
...
4. TPA system connector verification — FHIR PAS + X12 278 inbound rails are built (`docs/CONNECTOR_RAILS.md`); verify against the first named TPA's actual system, then build the sftp_batch rail for the long tail
