...
/**
 * Tests for the Canonical Intake Contract v1 (docs/INTAKE_CONTRACT.md) —
...
describe('gr-contract signing', () => {
  ...
  const DOC_SIG = 'a9b245c0cfb2c14711ad553ed871fda57d2723ee7586f8c3c5f30f1dcee2d2ed';
  ...
  it('verifies against the secondary secret (rotation without downtime)', () => {
    ...
    expect(verdict).toEqual({ ok: true, secretIndex: 1 });
  });
...
describe('POST /api/intake/voice — auth gate', () => {
  ...
  it('accepts a request signed with the SECONDARY secret during rotation', async () => {
    vi.stubEnv('GRAVITY_RAIL_WEBHOOK_SECRET', 'old-secret');
    vi.stubEnv('GRAVITY_RAIL_WEBHOOK_SECRET_SECONDARY', 'new-secret');
    const { POST } = await import('@/app/api/intake/voice/route');
    const raw = JSON.stringify(validBody());
