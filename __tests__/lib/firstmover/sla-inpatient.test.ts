...
import {
  ...
  FIRSTMOVER_SLA_HOURS,
} from '@/lib/firstmover/sla-inpatient';
...
describe('FIRSTMOVER_SLA_HOURS', () => {
  it("matches Santana's 2026-05-07 ops call values", () => {
    expect(FIRSTMOVER_SLA_HOURS.outpatient_standard).toBe(360);
    expect(FIRSTMOVER_SLA_HOURS.outpatient_expedited).toBe(72);
    expect(FIRSTMOVER_SLA_HOURS.inpatient_target).toBe(24);
    expect(FIRSTMOVER_SLA_HOURS.inpatient_notify_window).toBe(48);
    expect(FIRSTMOVER_SLA_HOURS.continued_stay_clinicals).toBe(72);
  });
});
