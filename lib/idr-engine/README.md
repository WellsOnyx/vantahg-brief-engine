...
**Optional — serve mode (many WorkSpaces, one server, zero installs).** Run `idr-batch` on one in-VPC machine, then `npx tsx scripts/idr-serve.ts <batch-output-dir>` with `IDR_SERVE_CODE` set. It serves the review queue and the mirror forms as simple web pages behind a shared access code, **bound to the private network only — it refuses any public/all-interface bind**. Arbiters open the printed URL from their WorkSpace browser; the API key and all processing stay on the serving machine. See `docs/IDR_ENGINE_SETUP.md`. (Roll out beyond one workstation only after it works the way you want on one.)
