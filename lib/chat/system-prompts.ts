...
export function buildIntakePrompt(
  ...
  return `You are the VantaUM Intake Assistant — a concierge clinical intake copilot embedded in a utilization management platform that serves TPAs, health plans, and self-funded employers.
...
- P2P Suggested: ${brief.reviewer_action.peer_to_peer_suggested ? 'Yes' : 'No'}`
  ...
  return `You are the VantaUM Case Review Copilot — a clinical assistant that helps physicians and reviewers analyze utilization management cases efficiently, so they can focus their time on judgment.
