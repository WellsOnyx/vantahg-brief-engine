    ...
    <div className="space-y-6">
        ...
        {fields.map((field, i) => (
          <div
            ...
            className={`bg-white rounded-xl border p-4 transition-all duration-500 ${
              i < filled ? 'border-[#c9a227]/40 shadow-sm opacity-100 translate-y-0' : 'border-transparent opacity-0 translate-y-2'
            }`}
          >
            <div className="text-[10px] uppercase tracking-wider text-[#0c2340]/40 font-semibold mb-1">{field.label}</div>
      ...
      {filled >= fields.length && (
        <div className="flex justify-center pt-4 animate-fadeIn">
          <button onClick={onNext} className="px-8 py-3 bg-[#0c2340] text-white font-semibold rounded-xl hover:bg-[#1a3a5c] transition-colors shadow-lg">
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 bg-[#c9a227] rounded-lg flex items-center justify-center text-[10px] font-bold text-[#0c2340]">V</div>
          <span className="text-sm font-semibold text-[#0c2340]">VantaUM Clinical Intelligence</span>
        </div>
        {typing ? (
          <div className="flex items-center gap-1 text-xs text-[#c9a227] font-medium"><span className="animate-pulse">●</span> Analyzing...</div>
        ) : (
          <span className="text-xs bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-full font-semibold">Verification Score: 94/100</span>
        )}
      </div>
      <div ref={containerRef} className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
        {BRIEF_SECTIONS.slice(0, visibleSections).map((section, i) => (
          <div key={section.id} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm animate-slideUp" style={{ animationDelay: `${i * 100}ms` }}>
              ...
              {section.badges && (
                <div className="flex gap-1.5 ml-auto">
                  {section.badges.map((b) => (
                    <span key={b.label} className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${b.type === 'source' ? 'bg-blue-50 text-blue-700' : b.type === 'met' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>{b.label}</span>
                  ))}
                </div>
              )}
            </div>
            {section.content && <p className="text-sm text-[#0c2340]/70 leading-relaxed whitespace-pre-line">{section.content}</p>}
            {section.criteria_met && (
              <ul className="space-y-1.5 mt-2">
                {section.criteria_met.map((c) => (
                  <li key={c} className="flex items-start gap-2 text-sm text-[#0c2340]/70"><span className="text-emerald-500 mt-0.5 flex-shrink-0">✓</span>{c}</li>
                ))}
              </ul>
            )}
            {section.missing && section.missing.length > 0 && (
              <div className="mt-3 p-2.5 bg-amber-50 rounded-lg border border-amber-200">
                <div className="text-[10px] uppercase tracking-wider text-amber-700 font-semibold mb-1">Missing Documentation</div>
                {section.missing.map((m) => (<p key={m} className="text-xs text-amber-800">{m}</p>))}
              </div>
            )}
            {section.recommendation && (
              <div className="mt-2 p-3 bg-emerald-50 rounded-lg border border-emerald-200">
                <div className="flex items-center gap-3 mb-2">
                  <span className="px-3 py-1 bg-emerald-600 text-white text-xs font-bold rounded-full uppercase tracking-wider">{section.recommendation}</span>
                  <span className="text-xs text-emerald-700 font-semibold">Confidence: {section.confidence}</span>
                </div>
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center gap-4 mb-4">
          <div className="w-14 h-14 bg-[#0c2340] rounded-full flex items-center justify-center text-white text-xl font-bold">JR</div>
          <div>
          ...
          <div className="ml-auto">
            {phase === 'assigning' && <span className="text-xs bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full font-semibold animate-pulse">Assigning...</span>}
            {phase === 'reviewing' && <span className="text-xs bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full font-semibold animate-pulse">Reviewing Brief...</span>}
            {phase === 'deciding' && <span className="text-xs bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-full font-semibold">Ready for Determination</span>}
          </div>
        </div>
        {phase !== 'assigning' && (
          <div className="space-y-3 animate-fadeIn">
            ...
            <div className="p-3 bg-[#f8f9fb] rounded-lg border text-sm text-[#0c2340]/70">
              <span className="font-semibold text-[#0c2340]">AI recommends: APPROVE</span> — All 6 InterQual criteria met. Physician reviews the AI brief and clinical documentation to make the final determination.
              <span className="block mt-1 text-[10px] uppercase tracking-wider text-[#0c2340]/40 font-semibold">AI advises. Physicians decide.</span>
            </div>
          </div>
        )}
      </div>
      {phase === 'deciding' && (
        <div className="flex justify-center pt-2 animate-fadeIn">
          <button onClick={onNext} className="px-8 py-3 bg-emerald-600 text-white font-semibold rounded-xl hover:bg-emerald-700 transition-colors shadow-lg">
      ...
      <div className="bg-white rounded-xl border-2 border-emerald-200 p-6 shadow-lg">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-[#0c2340]/60">Case {DEMO_CASE.case_number} — {DEMO_CASE.procedure_description}</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><div className="text-[10px] uppercase tracking-wider text-[#0c2340]/40 font-semibold">Authorization #</div><div className="font-mono font-bold text-[#0c2340]">AUTH-2026-004821</div></div>
          <div><div className="text-[10px] uppercase tracking-wider text-[#0c2340]/40 font-semibold">Determined By</div><div className="font-medium text-[#0c2340]">{REVIEWER.name}, {REVIEWER.credentials}</div></div>
          <div><div className="text-[10px] uppercase tracking-wider text-[#0c2340]/40 font-semibold">Turnaround</div><div className="font-medium text-emerald-600">1h 47m (SLA: 48h)</div></div>
          <div><div className="text-[10px] uppercase tracking-wider text-[#0c2340]/40 font-semibold">AI Agreement</div><div className="font-medium text-emerald-600">Physician agrees with AI recommendation</div></div>
        </div>
        <div className="p-3 bg-emerald-50 rounded-lg text-sm text-emerald-800">
          <strong>Rationale:</strong> All InterQual criteria for lumbar spine MRI met. Progressive radiculopathy with objective neurological findings persisting beyond 6 weeks despite documented conservative management. Authorization approved for outpatient MRI lumbar spine without contrast.
        </div>
      </div>
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border p-4 text-center"><div className="text-3xl font-bold text-[#0c2340]">96%</div><div className="text-[10px] uppercase tracking-wider text-[#0c2340]/40 font-semibold mt-1">Time Saved vs Manual</div></div>
        <div className="bg-white rounded-xl border p-4 text-center"><div className="text-3xl font-bold text-[#c9a227]">1:47</div><div className="text-[10px] uppercase tracking-wider text-[#0c2340]/40 font-semibold mt-1">Total Minutes</div></div>
        <div className="bg-white rounded-xl border p-4 text-center"><div className="text-3xl font-bold text-emerald-600">100%</div><div className="text-[10px] uppercase tracking-wider text-[#0c2340]/40 font-semibold mt-1">Audit-Ready</div></div>
      </div>
      <div className="flex justify-center pt-2">
        <button onClick={onRestart} className="px-8 py-3 bg-[#0c2340] text-white font-semibold rounded-xl hover:bg-[#1a3a5c] transition-colors shadow-lg">Watch Again</button>
      </div>
    </div>
  );
}
...
export default function DemoWalkthrough() {
  ...
  const restart = useCallback(() => { setStep(0); setKey((k) => k + 1); }, []);

  return (
    <div className="bg-[#f8f9fb] rounded-xl p-4 sm:p-8" key={key}>
