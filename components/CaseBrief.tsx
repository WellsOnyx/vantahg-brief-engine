        ...
        {[1, 2, 3].map((i) => (
          <div
            ...
            className={`w-5 h-2 rounded-sm ${
              i <= levels[confidence] ? colors[confidence] : 'bg-gray-200'
            }`}
          />
        ))}
      </div>
      <span className="text-xs font-medium text-muted">{labels[confidence]}</span>
    </div>
  );
}
...
function RecommendationBadge({
  ...
  return (
    <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border font-semibold text-sm ${bg}`}>
      {icon}
      {label}
    </div>
  );
...
export function CaseBrief({ brief, caseNumber }: CaseBriefProps) {
  return (
    <div className="bg-surface rounded-lg border border-border">
      {/* Header */}
      <div className="px-6 py-4 border-b border-border bg-navy/[0.02]">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 no-print">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md border border-border bg-white hover:bg-gray-50 text-foreground transition-colors"
            >
      ...
      <div className="px-6 py-5 space-y-0">
        {/* 1. Clinical Question */}
        <div className="p-4 rounded-lg bg-gold/[0.06] border border-gold/20">
          <div className="flex items-start gap-3">
            <svg className="w-5 h-5 text-gold-dark shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {brief.procedure_analysis.codes.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              {brief.procedure_analysis.codes.map((code) => (
                <span
                  key={code}
                  className="inline-flex items-center px-2 py-1 bg-gray-100 border border-gray-200 rounded text-xs font-mono font-medium text-foreground"
                >
                  {code}
                </span>
              ))}
        ...
        <div>
          ...
          <p className="text-xs text-muted mb-3">
            Guideline: <span className="font-medium text-foreground">{brief.criteria_match.applicable_guideline}</span>
          </p>
            ...
            {brief.criteria_match.criteria_met.length > 0 && (
              <div className="space-y-1">
                {brief.criteria_match.criteria_met.map((criterion, idx) => (
                  <div key={idx} className="flex items-start gap-2 p-2 rounded bg-green-50 border border-green-100">
            ...
            {brief.criteria_match.criteria_not_met.length > 0 && (
              <div className="space-y-1">
                {brief.criteria_match.criteria_not_met.map((criterion, idx) => (
                  <div key={idx} className="flex items-start gap-2 p-2 rounded bg-red-50 border border-red-100">
            ...
            {brief.criteria_match.criteria_unable_to_assess.length > 0 && (
              <div className="space-y-1">
                {brief.criteria_match.criteria_unable_to_assess.map((criterion, idx) => (
                  <div key={idx} className="flex items-start gap-2 p-2 rounded bg-amber-50 border border-amber-100">
                    <span className="w-4 h-4 text-amber-600 shrink-0 mt-0.5 flex items-center justify-center font-bold text-xs">
                      ?
                    </span>
        ...
        <div>
          ...
          <p className="text-sm text-muted mb-3">
            Documents provided: <span className="text-foreground">{brief.documentation_review.documents_provided}</span>
          </p>
        ...
        {/* 6. AI Recommendation */}
        <div className="p-4 rounded-lg bg-navy/[0.03] border border-navy/10">
        ...
        <div className="p-4 rounded-lg border-2 border-gold/40 bg-gold/[0.04]">
          <div className="flex items-center gap-2 mb-3">
            <svg className="w-5 h-5 text-gold-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              {brief.reviewer_action.peer_to_peer_suggested && (
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-purple-100 border border-purple-200">
            ...
            {brief.reviewer_action.additional_info_needed.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-gold-dark uppercase tracking-wider mb-1.5">
