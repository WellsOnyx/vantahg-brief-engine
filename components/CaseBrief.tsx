...
export function CaseBrief({ brief, caseNumber, factCheck }: CaseBriefProps) {
 ...
 <h2 className="text-xl text-navy">
                ...
                <p className="text-xs text-muted mt-0.5">
                  Case {caseNumber} &middot; Prepared by VantaUM
                </p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3 no-print">
            ...
            {brief.generation_metadata?.self_improvement_applied && (
              <div
                ...
                title={`AI performed ${brief.generation_metadata.passes_completed} self-critique passes before surfacing for human validation. Final score lift: +${(brief.generation_metadata.final_fact_check_score ?? 0) - (brief.generation_metadata.initial_fact_check_score ?? 0)} pts.`}
              >
                <span>✧</span>
                <span>Self-refined • {brief.generation_metadata.passes_completed} passes</span>
              </div>
            )}
            <button
            ...
            <details className="group rounded-xl border border-emerald-200 bg-emerald-50/40 px-4 py-3 text-sm">
              <summary className="flex cursor-pointer items-center justify-between font-semibold text-emerald-800 select-none">
                <span className="flex items-center gap-2">
                  ...
                  AI Self-Improvement Log ({brief.generation_metadata.passes_completed} passes)
                  <span className="text-[10px] font-normal text-emerald-700/70">
                    +{(brief.generation_metadata.final_fact_check_score ?? 0) - (brief.generation_metadata.initial_fact_check_score ?? 0)}pt defensibility lift
                  </span>
                </span>
              ...
              <div className="mt-3 space-y-3 border-t border-emerald-200/70 pt-3 text-emerald-900">
                ...
                {brief.generation_metadata.revisions?.map((rev, idx) => (
