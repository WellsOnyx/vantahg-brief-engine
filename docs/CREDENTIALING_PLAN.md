...
**Phase 1 — Data model + intake — ✅ SHIPPED.** Migration 035: `providers`, `credentialing_cases` (one active cycle per provider, DB-enforced), `verification_items` (one per NCQA element, unique per cycle), `monitoring_subscriptions`. Partner intake at `POST /api/partner/v1/credentialing/providers` (partner key + Idempotency-Key ledger, same contract discipline as case submit) + status read with tenant wall. NCQA element set as config (`lib/credentialing/config.ts` — DEA prescribers-only, board cert MD/DO-only, 36-month cycle). PSV orchestration seam (`lib/credentialing/psv.ts`): seeds items idempotently, adapter contract with CAQH stub (activates on `CAQH_API_KEY`) + structured manual fallback — an unverifiable element is a visible pending row, never a silent skip. Committee decision endpoint (`POST /api/credentialing/cases/[id]/decision`) is the ONLY writer of `decision`: staff-only, blocked until every required element is terminal, discrepancies require `attestation.flags_acknowledged`, rationale ≥ 30 chars, one decision per cycle. 11 tests lock the wall.
